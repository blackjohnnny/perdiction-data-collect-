...
function runStrategy(strategyName, cooldownBehavior) {
  ...
  for (let i = 0; i < rounds.length; i++) {
    ...
    // Cap bet amount to prevent unrealistic growth
    const cappedBetAmount = Math.min(betAmount, bankroll * 0.2); // Max 20% of bankroll per trade

    const actualPayout = signal === 'BULL' ? bullPayout : bearPayout;
    const won = (signal === 'BULL' && winner === 'bull') || (signal === 'BEAR' && winner === 'bear');
    const profit = won ? cappedBetAmount * (actualPayout - 1) : -cappedBetAmount;
...
console.log('🔬 PROPER COMPARISON: Circuit Breaker Strategies\n');
console.log('════════════════════════════════════════════════════════════════════════════\n');
