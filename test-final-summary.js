...
async function test() {
  ...
  // ONLY use rounds with T-20s data (420 rounds)
  ...
  console.log(`\n📊 FINAL SUMMARY - ALL 420 COMPLETE ROUNDS\n`);
  console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`);

  // Strategy 1: HYBRID ADAPTIVE (original in consolidation, original in trending)
  const hybridStrategy = (marketCondition, bullPercent, bearPercent) => {
    if (marketCondition.isConsolidation) {
      // CONSOLIDATION: Bet WITH crowd (original)
    ...
    } else if (marketCondition.isTrending) {
      // TRENDING: Bet WITH crowd (original)
      if (marketCondition.emaSignal === 'BULL' && bullPercent >= CROWD_THRESHOLD) {
        return { betSide: 'BULL', isConsolidationTrade: false };
      }
      if (marketCondition.emaSignal === 'BEAR' && bearPercent >= CROWD_THRESHOLD) {
        return { betSide: 'BEAR', isConsolidationTrade: false };
      }
    }
    return null;
  };

  // Strategy 2: ORIGINAL (always bet WITH crowd)
  const originalStrategy = (marketCondition, bullPercent, bearPercent) => {
    const isConsolidation = marketCondition.isConsolidation;
    if (marketCondition.emaSignal === 'BULL' && bullPercent >= CROWD_THRESHOLD) {
      return { betSide: 'BULL', isConsolidationTrade: isConsolidation };
    }
    if (marketCondition.emaSignal === 'BEAR' && bearPercent >= CROWD_THRESHOLD) {
      return { betSide: 'BEAR', isConsolidationTrade: isConsolidation };
    }
    return null;
  };

  // Strategy 3: REVERSE (always bet AGAINST crowd)
  ...
  const hybridResult = await runStrategy(rounds, 'HYBRID', hybridStrategy);
  console.log(`\n🎯 HYBRID ADAPTIVE (Original WITH crowd in ALL conditions):`);
  console.log(`├─ Total trades: ${hybridResult.totalTrades}`);
  console.log(`│  ├─ Consolidation: ${hybridResult.consolidationTrades} trades (${hybridResult.consolidationWins} wins, ${hybridResult.consolidationWinRate}% win rate)`);
  console.log(`│  └─ Trending: ${hybridResult.trendingTrades} trades (${hybridResult.trendingWins} wins, ${hybridResult.trendingWinRate}% win rate)`);
  console.log(`├─ Overall Win Rate: ${hybridResult.winRate}%`);
  console.log(`├─ ROI: ${hybridResult.roi}%`);
  console.log(`└─ Final Balance: ${hybridResult.finalBalance} BNB\n`);

  const originalResult = await runStrategy(rounds, 'ORIGINAL', originalStrategy);
  console.log(`\n✅ ORIGINAL (Always bet WITH crowd):`);
  console.log(`├─ Total trades: ${originalResult.totalTrades}`);
  console.log(`├─ Win Rate: ${originalResult.winRate}%`);
  ...
  console.log(`\n❌ REVERSE (Always bet AGAINST crowd):`);
  console.log(`├─ Total trades: ${reverseResult.totalTrades}`);
  console.log(`├─ Win Rate: ${reverseResult.winRate}%`);
  ...
  console.log(`  Original (WITH crowd): 66.67% win rate ✅`);
  console.log(`  Reverse (AGAINST crowd): 33.33% win rate ❌\n`);
  console.log(`Trending performance:`);
  console.log(`  Original (WITH crowd): 53.85% win rate`);
  console.log(`  Reverse (AGAINST crowd): 46.15% win rate\n`);
