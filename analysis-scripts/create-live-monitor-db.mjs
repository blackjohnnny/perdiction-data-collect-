...
const oldBuf = fs.readFileSync('../data/prediction-data.db');
const oldDb = new sqlJs.Database(oldBuf);

// Create new live monitor database
console.log('Creating live-monitor.db...');
const newDb = new sqlJs.Database();

// Create schema with snapshots as columns
newDb.run(`
  CREATE TABLE rounds (
...
// Get rounds that have ANY snapshot data (these are monitored rounds)
const query = `
  SELECT DISTINCT r.*
  FROM rounds r
  JOIN snapshots s ON r.epoch = s.epoch
  WHERE s.snapshot_type IN ('T_MINUS_20S', 'T_MINUS_25S', 'T_MINUS_8S', 'T_MINUS_4S')
  ORDER BY r.epoch
`;

const roundsResult = oldDb.exec(query);
if (roundsResult.length === 0) {
  console.log('No monitored rounds found');
  process.exit(0);
}
...
console.log(`Found ${roundRows.length} monitored rounds to migrate`);
...
console.log(`\n=== Migration Complete ===`);
console.log(`Total monitored rounds: ${inserted}`);
...
fs.writeFileSync('../data/live-monitor.db', buffer);

console.log('\n✅ Live monitor database saved as: live-monitor.db');

// Show summary
const withResults = newDb.exec('SELECT COUNT(*) FROM rounds WHERE winner != \"UNKNOWN\"')[0].values[0][0];
const t20Results = newDb.exec('SELECT COUNT(*) FROM rounds WHERE t20s_total_wei IS NOT NULL AND winner != \"UNKNOWN\"')[0].values[0][0];
const t8Results = newDb.exec('SELECT COUNT(*) FROM rounds WHERE t8s_total_wei IS NOT NULL AND winner != \"UNKNOWN\"')[0].values[0][0];
const t4Results = newDb.exec('SELECT COUNT(*) FROM rounds WHERE t4s_total_wei IS NOT NULL AND winner != \"UNKNOWN\"')[0].values[0][0];

console.log('\n=== Live Monitor Database Summary ===');
console.log(`Total monitored rounds: ${inserted}`);
console.log(`With settled results: ${withResults}`);
console.log(`\nComplete data (snapshots + results):`);
console.log(`  T-20s: ${t20Results} rounds`);
console.log(`  T-8s: ${t8Results} rounds`);
console.log(`  T-4s: ${t4Results} rounds`);
...
console.log('\n=== Latest 10 monitored rounds ===');
console.log('Epoch    | Winner | T-20s | T-8s | T-4s');
console.log('---------|--------|-------|------|------');
sample[0].values.forEach(row => {
  console.log(`${row[0]} | ${row[1].padEnd(6)} | ${row[2].padEnd(5)} | ${row[3].padEnd(4)} | ${row[4]}`);
});
